...
def get_fcst_params(d, Nhrs, ij):

    hs = np.empty([Nhrs, len(ij)])
    dp = np.empty([Nhrs, len(ij)])
    tp = np.empty([Nhrs, len(ij)])

    for k in range(len(ij)):
        ilon = ij[k][0]
        ilat = ij[k][1]
        hs[:, k] = d['htsgwsfc'][:Nhrs, ilat, ilon].data
        dp[:, k] = d['dirpwsfc'][:Nhrs, ilat, ilon].data
        tp[:, k] = d['perpwsfc'][:Nhrs, ilat, ilon].data

    tidx = nc.num2date(d['time'][:Nhrs], units=d['time'].units)
